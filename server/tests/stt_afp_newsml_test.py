...
class AFPNewsMLFeedParserTestCase(TestCase):
    ...
    async def test_prefixes_first_advisory_line_to_body_html(self):
        item = await self.parse_item_from_fixture("afp_test.xml")
    ...
    async def test_does_not_prefix_when_newslinetype_is_not_advisoryline(self):
        item = await self.parse_item_from_fixture("afp_wrong_newslinetype_name.xml")

        self.assertTrue(item.get("body_html"))
        # The fixture uses ProductLine with NewsLineText "(Video)".
        self.assertFalse(item["body_html"].startswith("<p>(Video)</p>\n"))
        self.assertTrue(item["body_html"].startswith("<p>Indonesian authorities"))

    async def test_does_not_prefix_when_no_newslinetype(self):
        item = await self.parse_item_from_fixture("afp_no_newsline_type.xml")

        self.assertTrue(item.get("body_html"))
        # No NewsLineType exists, so no extra prefix should be injected.
        self.assertTrue(item["body_html"].startswith("<p>Trump says"))
...
<NewsML Version="1.2">
  <NewsItem>
    ...
    <NewsManagement>
      <FirstCreated>20260109T091444+0000</FirstCreated>
      <ThisRevisionCreated>20260109T091444+0000</ThisRevisionCreated>

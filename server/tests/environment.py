...
from settings import INSTALLED_APPS, MODULES
...
async def before_scenario_async(context, scenario):
    ...
    if "stt_providers" in scenario.tags:
        await setup_stt_providers(context)
...
async def mock_fetch_ingest(self: FileFeedingService, guid: str):
    path_to_fixtures = path.join(
        path.abspath(path.dirname(__file__)), "../tests/fixtures"
    )
    file_path = path.join(path_to_fixtures, guid)
    feeding_parser = await self.get_feed_parser(self.provider)

    with open(file_path, "rb") as f:
        xml_string = etree.etree.fromstring(f.read())
        return await feeding_parser.parse(xml_string, self.provider)


async def setup_stt_providers(context):
    app = context.app
    context.providers = {}

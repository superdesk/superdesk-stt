...
class STTNewsmLG2Formatter(NewsMLG2Formatter):
    ...
    def format_texttype(self, article, parentNode):
        ...
        for s in subjects:
            if s.get("scheme", None) == "sttgenre":
                SubElement(
                    parentNode,
                    "genre",
                    attrib={"qcode": "sttgenre:" + s.get("qcode", "")},
                ).text = s.get("name", "")
    ...
    def format_contentMeta(self, article, parentNode):
        ...
        if article.get("body_html", None):
            ...
            SubElement(
                contentMeta,
                "related",
                attrib={"qcode": "sttrel:actuallength", "value": str(characterCount)},
            )
    ...
    def format_contentSet(self, article, parentNode):
        ...
        if extra:
            if extra.get("sttsubheadline", None):
                ...
                # Find the paragrap and get the content to H2
        ...
        if article.get("body_html", None):
            ...
            # parser = etree.XMLParser()
            # tree = etree.XML('<div>' + article.get('body_html', '') + '</div>', parser)

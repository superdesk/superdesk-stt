...
class STTNewsmLG2Formatter(NewsMLG2Formatter):
            ...
            match state:
                case 'corrected':
                    if 'original_id' in article:
                        ...
                        SubElement(signal, 'link', attrib={'guidref': 'urn:newsml:stt:fi::' + article.get('guid', ''), 'version': str(versionNumber)})
                case _:
    ...
    def format_location(self, article, parentNode):
        ...
        for s in places:
            if s.get('name', None):
                location = SubElement(parentNode, 'located', attrib={'type': s.get('qcode', '')})
    ...
    def format_assignments(self, article, parentNode):
        ...
        if article.get('assignment_id', None):
            ...
            assignment = superdesk.get_resource_service('assignments').find_one(req=None, _id=article.get('assignment_id'))

            if assignment is not None:
                if assignment.get('planning', None):
                    coverageStatus = assignment.get('planning', '')['news_coverage_status']['label']
                    ...
                    SubElement(parentNode, 'subject', attrib={'type': 'cpnat:abstract', 'literal': 'Related topic id', 'qcode': 'stt-topics:' + assignment.get('planning_item', '')})
                    SubElement(parentNode, 'subject', attrib={'type': 'cpnat:abstract', 'qcode': 'sttdone1:' + coverageStatusCode})
    ...
    def format_IPTC(self, article, parentNode):
        ...
        for s in subjects:
            if s.get('scheme', None) == 'topics':
                subj = SubElement(parentNode, 'subject', attrib={'type': 'cpnat:abstract', 'qcode': s.get('scheme', '') + ':' + s.get('qcode', '')})
    ...
    def format_texttype(self, article, parentNode):
        ...
        if article.get('genre'):
            for s in article['genre']:
                genre = SubElement(parentNode, 'genre', attrib={'qcode': s.get('qcode', '')})
    ...
    def format_versiontype(self, article, parentNode):
        ...
        if versiontype:
            ...
            match versiontype:
                ...
                case 'viiva':
                    genre = SubElement(parentNode, 'genre', attrib={'qcode': 'sttversion:1'})
                    SubElement(genre, 'name').text = 'Viiva'

                    # Character count
                    characterCount = text_utils.get_char_count(article.get('headline', ''))
                    SubElement(genre, 'related', attrib={'qcode': 'sttrel:actuallength', 'value': str(characterCount)})

                case 'sms':
                    genre = SubElement(parentNode, 'genre', attrib={'qcode': 'sttversion:2'})
                    SubElement(genre, 'name').text = 'SMS'

                case 'pika':
                    genre = SubElement(parentNode, 'genre', attrib={'qcode': 'sttversion:3'})
                    SubElement(genre, 'name').text = 'Pika'

                    # Character count
                    if article.get('body_html', None):
                        characterCount = text_utils.get_char_count(article.get('body_html', ''))
                        SubElement(genre, 'related', attrib={'qcode': 'sttrel:actuallength', 'value': str(characterCount)})

                case 'pikaplus':
                    genre = SubElement(parentNode, 'genre', attrib={'qcode': 'sttversion:4'})
                    SubElement(genre, 'name').text = 'Pika+'
                ...
                case 'nettiuutinen':
                    genre = SubElement(parentNode, 'genre', attrib={'qcode': 'sttversion:6'})
    ...
    def format_topstory(self, article, parentNode):
        ...
        for s in subjects:
            if s.get('scheme', None) == 'stttopstory':
                ...
                SubElement(parentNode, 'subject', attrib={'type': 'cpnat:abstract', 'qcode': 'stttopstory:' + topStoryYesNo})
    ...
    def format_images(self, article, parentNode):
        ...
        if associations:
            ...
            if featureMedia:
                link = SubElement(parentNode, 'link', attrib={'rel': 'seeAlso', 'residref': featureMedia.get('media', ''), 'contenttype': featureMedia.get('mimetype', '')})
                SubElement(link, 'title', attrib={'role': 'drol:caption'}).text = featureMedia.get('description_text', '')

                match featureMedia.get('mimetype', ''):
                    case "image/png":
                        SubElement(link, 'filename').text = featureMedia.get('media', '') + ".png"
                    case "image/jpeg":
                        SubElement(link, 'filename').text = featureMedia.get('media', '') + ".jpg"
                    case "image/jpg":
                        SubElement(link, 'filename').text = featureMedia.get('media', '') + ".jpg"
    ...
    def format_itemMeta(self, article, parentNode):
        ...
        SubElement(itemMeta, "versionCreated").text = article.get('versioncreated', None).astimezone(pytz.timezone('Europe/Helsinki')).strftime("%Y-%m-%dT%H:%M:%S")
        SubElement(itemMeta, "firstCreated").text = article.get('firstcreated', None).astimezone(pytz.timezone('Europe/Helsinki')).strftime("%Y-%m-%dT%H:%M:%S")

        # Embargo
        schedule = article.get('schedule_settings', None)
        if schedule:
            if 'utc_embargo' in schedule:
                SubElement(itemMeta, 'embargoed').text = schedule.get('utc_embargo', None).astimezone(pytz.timezone('Europe/Helsinki')).strftime("%Y-%m-%dT%H:%M:%S")
        ...
        for s in subjects:
            if s.get('scheme') == 'sttnewsroomnote':
                ...
                SubElement(itemMeta, 'edNote', attrib={'role': 'sttnewsroomnote'}).text = s.get('name', '')
        ...
        if 'ednote' in article:
            ...
            SubElement(itemMeta, 'edNote', attrib={'role': 'sttnote:private'}).text = edNoteStr
            ...
            if extra:
                if 'sttpublicednote' in extra:
                    ...
                    if extra.get('sttpublicednote', None):
                        ...
                        SubElement(itemMeta, 'edNote', attrib={'role': 'sttnote:public'}).text = element.text
        ...
        for element in tree.xpath('//a'):
            SubElement(itemMeta, 'link', attrib={'rel': 'irel:seeAlso', 'href': element.get('href')}).text = element.text
    ...
    def format_contentMeta(self, article, parentNode):
        ...
        SubElement(contentMeta, 'contentCreated').text = article.get('versioncreated', None).astimezone(pytz.timezone('Europe/Helsinki')).strftime("%Y-%m-%dT%H:%M:%S")
        SubElement(contentMeta, 'contentModified').text = article.get('firstcreated').astimezone(pytz.timezone('Europe/Helsinki')).strftime("%Y-%m-%dT%H:%M:%S")
        SubElement(contentMeta, 'altId', attrib={'type': 'sttidtype:textid'}).text = article.get('guid', '')
        ...
        SubElement(contentMeta, 'altId', attrib={'type': 'sttidtype:runningnumber', 'environment': 'sttcounter:daily'}).text = str(article.get('publish_sequence_no', ''))
        ...
        if dateline is not None:
            ...
            if located is not None:
                ...
                if city is not None:
                    SubElement(contentMeta, 'dateline').text = article.get('dateline', '').get('located', '').get('city', '')
    ...
    def format_contentSet(self, article, parentNode):
        ...
        inlineXML = SubElement(contentSet, 'inlineXML', attrib={'contenttype': 'xhtml/xml'})
        ...
        if article.get('body_html', None):
            ...
            # Find all rows
            rows = re.findall(r"<tr.*?>(.*?)</tr>", article.get('body_html'), flags=re.DOTALL)
            ...
            if bodyHtml:
                resultHTML = re.sub('<table.*?>.*?</table>', replacement, bodyHtml, flags=re.DOTALL)
                ...
                for element in tree.xpath('//span[@custom-editor-tag-id="EDITOR_TAG_company"]'):
                ...
                for element in tree.xpath('//span[@custom-editor-tag-id="EDITOR_TAG_person"]'):
    ...
    async def format(self, article, subscriber, codes=None):
        ...
        try:
            ...
            newsItem = etree.Element("newsItem", attrib={
            ...
            SubElement(newsItem, "catalogRef", attrib={"href": "http://www.iptc.org/std/catalog/catalog.IPTC-G2-Standards_19.xml"})
            SubElement(newsItem, "catalogRef", attrib={"href": "http://www.iptc.org/std/catalog/catalog.IPTC-G2-Standards_18.xml"})
            SubElement(newsItem, "catalogRef", attrib={"href": "http://www.stt-lehtikuva.fi/newsml/doc/stt-NewsCodesCatalog_1.xml"})
